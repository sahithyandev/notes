...
lastUpdatedOn: 2026-06-12T06:46:09.150Z
...
The GPIO pin sources only $I_B = I_f / h_{FE}$, well within pin limits.
...
As $n$ increases, $V_{segment}$ falls. With enough segments active, $V_{segment}$ drops below $V_f$, extinguishing segments entirely.
...
Spike amplitude depends on $L$ and $\text{d}I/\text{d}t$. A fast switch-off generates spikes of tens to hundreds of volts, above the MCU's rated supply. Without protection, the spike destroys the transistor and can permanently damage the MCU.
...
Turning on Q1 and Q2 simultaneously (or Q3 and Q4) shorts $V_{CC}$ to GND and instantly destroys the switches.
