...
lastUpdatedOn: 2026-09-12T09:34:04.947Z
prereqs:
  - s2/data-structures-and-algorithms/algorithms/complexity-analysis
---
...
- Vertices  
  Locations or cities.
- Edges  
  Paths between locations. May be directed.
- Weights  
  Cost, distance, or time.

## Travelling Salesman Problem

The problem of finding the shortest possible closed walk that visits every vertex at least once and returns to the starting point. No repeating edges. Multiple valid optimal solutions may exist.
...
Solving the problem by trying all possible solutions and selecting the optimal one. May start at any vertex. Simple. Guaranteed to be optimal. Computationally very expense. Not suitable for real world applications.
...
- For each permutation:
  - Construct the route as A → permutation → A
...
A greedy method. Builds a route by repeatedly visiting the closest unvisited vertex. Sensitive to starting vertex. Fast. Simple. Not necessarily optimal.
...
A dynamic programming method. Solves the problem by breaking it into smaller subproblems and combining their optimal solutions. Guaranteed to be optimal. More optimal than brute force. Still exponential in nature. High memory usage. Impractical for very large $n$.

Only applicable for connected graphs. If not complete, missing edges must be added (with high weights).
...
Aka. Route Inspection Problem. The problem of finding the shortest closed path that traverses every edge of a graph at least once. Defined on weighted undirected connected graphs.
...
- Find all vertices with odd degree (must be even numbered by [the handshaking lemma](/s4/graph-theory/handshaking-lemma#corollary))
- If none, the Eulerian circuit already exists, so find it
- Else:
