...
lastUpdatedOn: 2026-06-04T14:41:47.355Z
---
...
The actual set of protocols that powers the modern internet. Developed around the same time as OSI. Fewer layers, easier to implement.
...
- Prior deployment  
  Already running on the internet before OSI was finalized.
- Universal support  
  Runs on everything: supercomputers, PCs, phones. Any device joins the same network without translation layers.
- Consistent APIs  
  BSD sockets became the universal interface; applications written once run across platforms.
- Strong track record  
  Proven at global scale over decades with no fundamental architectural failures.
- Simpler design  
  Fewer layers reduce overhead and implementation complexity compared to OSI.
- Internet foundation  
  Underpins the internet and World Wide Web; network effects make alternatives impractical to adopt.

The physical layer is provided outside TCP/IP. Session and presentation functions are absorbed into other layers.

### SNA

Aka. Systems Network Architecture. IBM's proprietary networking architecture, developed in the 1970s. For connecting IBM mainframes and terminals. It was widely used in corporate environments but was largely replaced by TCP/IP over time.

### WAP

Aka. Wireless Application Protocol. Developed in the late 1990s. Allows early mobile phones to access internet-like content. Predated smartphones and was designed for the very limited screens and slow connections at that time. Became mostly obsolete once modern smartphones with full browsers arrived.
