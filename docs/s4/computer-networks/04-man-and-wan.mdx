...
lastUpdatedOn: 2026-06-04T20:58:06.728Z
prereqs:
...
- Fixed monthly cost; not usage-based.
- Types: analog or digital. Most modern lines are digital.
- Common speeds: 64 kb/s (few computers), 2 Mb/s (large office).

Used in:

- Voice: tie lines between PBXs (Private Branch Exchanges, private telephone switches inside organizations that manage internal calls and connect to the public phone network). Tie lines are dedicated links between two PBXs so inter-office calls bypass the public phone network.
- Data: corporate networks, Internet access.
- Combined voice + data.

## X.25

International Telecommunication Union (ITU) standard for user interface to a packet data network. ITU is the UN agency that sets global telecom standards.
