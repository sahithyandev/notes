...
In software testing, a program is executed with artificial data to check for errors, anomalies, or non-functional issues. Dynamic verification.

Testing can reveal the **presence** of errors, but NOT their absence.

Easily reproducible. However, certain errors may be masked by other errors.
...
Inputs are grouped into equivalence partitions: groups of inputs the program treats equivalently. Test cases are picked from each group. One representative per partition is sufficient.

## Guideline-based Testing

Uses experience of common programmer errors to select test cases. For sequences: test single values, sequences of different sizes, access first/middle/last elements, test zero-length sequences.
...
Tests are written before the code. The tests function as the specification initially and then as the regression test suite.
