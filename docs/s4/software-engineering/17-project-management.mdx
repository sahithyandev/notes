...
<Note>
  {" "}
  Dorothy case study (slides 29–30): illustrates self-realisation deficit —
...
- Estimation by analogy  
  Compare to a similar completed project; adjust the estimate by difference factors.
- Algorithmic cost modelling  
  Mathematical formula using quantified software metrics as inputs. COCOMO II is the primary example.
- Parkinson's Law  
  Work expands to fill available time and budget. Not an estimation method; explains overruns.
- Price to Win  
  Estimate set to secure the contract, independent of actual cost. Leads to loss-making contracts.

### COCOMO II

COCOMO II (COnstructive COst MOdel) estimates effort in person-months.

| Model                   | When Used                                                                           |
| ----------------------- | ----------------------------------------------------------------------------------- |
| Application Composition | Prototyping and reuse-heavy projects. Formula: PM = (NAP × (1 − %reuse/100)) / PROD |
| Early Design            | After requirements agreed, before design starts. Formula: PM = A × Size^B × M       |
| Reuse Model             | Computing effort to integrate reusable components (black-box vs white-box reuse).   |
| Post-Architecture       | After architecture designed; most accurate. Uses 17 multipliers.                    |

Key multipliers (M): RCPX (reliability/complexity), RUSE (reuse required), PDIF (platform difficulty), PREX (personnel experience), PERS (personnel capability), SCED (required schedule), FCIL (team support facilities).
...
Each function is rated simple, average, or complex to produce an unadjusted function count (UFC). UFC is then scaled by a complexity adjustment factor (CAF) derived from 14 general system characteristics.
