...
lastUpdatedOn: 2026-09-25T06:53:36.572Z
---

### Properties of Sorting

- Comparison / Non Comparison  
  In non-comparison based sorting, elements are sorted from their intrinsic properties instead.
- In-place/Outplace  
  Whether extra memory is used while sorting. In-place algorithms have $O(1)$ space complexity.
- Online/Offline  
  A sorting is considered online if it can accept new data while the procedure is ongoing.
- Stable/Unstable technique  
  A sorting is stable if the order of elements with the same value is preserved while sorting.
...
Uses a [binary heap](/s2/data-structures-and-algorithms/data-structures/heap/) data structure. Operates in two main phases:
