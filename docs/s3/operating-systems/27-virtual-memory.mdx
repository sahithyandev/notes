...
lastUpdatedOn: 2026-09-13T14:55:23.537Z
---
...
Virtual memory is divided into fixed-size _pages_. Page size is equal to [frame](/s3/computer-architecture/memory/) size. Typically 4 KB or 8 KB. There can be more pages than frames.
...
[TLBs](/s3/computer-architecture/memory#tlb) are used to speed up page table lookups.
...
- Page-out  
  Pages are written (swapped) from RAM to swap space.
- Page-in  
  When swapped pages are needed again, they're read back into RAM.
...
If a process is currently doing an I/O operation, it cannot be swapped out. Can be solved by [double buffering](/s3/operating-systems/io-system#double-buffering).
