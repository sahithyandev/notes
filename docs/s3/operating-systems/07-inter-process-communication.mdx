...
lastUpdatedOn: 2026-09-13T04:35:59.901Z
---
...
- Zero  
  No messages can be queued. Sender and receiver must be ready at the same time.
- Bounded  
  Queue has a fixed size. Sender waits if full. Receiver waits if empty.
- Unbounded  
  Queue has no size limit. Sender never waits. Receiver waits if empty.
