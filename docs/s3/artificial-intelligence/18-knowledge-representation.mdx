...
lastUpdatedOn: 2026-09-13T14:55:23.016Z
---
...
- Disjoint  
  No shared members: $\text{Disjoint({Animals, Vegetables})}$
- Exhaustive decomposition  
  Must belong to one: $\text{ExhaustiveDecomposition({Americans, Canadians, Mexicans}, NorthAmericans)}$
- Partition  
  Both disjoint and exhaustive: $\text{Partition({Males, Females}, Animals)}$
...
- Action  
  Logical term representing activity (e.g., $\text{Turn}(\text{Right}))$.
- Fluent  
  Predicate/function varying over time.
- Atemporal predicates  
  Permanent truths (e.g., $\text{Gold}(\text{G1})$).
