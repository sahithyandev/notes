...
lastUpdatedOn: 2026-09-13T04:35:59.511Z
---
...
3. Apply blocking rules
   - For a chain or fork, the path is blocked **if** the middle node is conditioned.
   - For a collider, the path is blocked **if** both the collider and its descendents are not conditioned.
