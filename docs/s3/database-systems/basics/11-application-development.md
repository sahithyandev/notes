...
sidebar:
  order: 11
slug: s3/database-systems/basics/application-development
prev: true
next: true
dateCreated: 2026-04-22T07:09:10.695Z
lastUpdatedOn: 2026-04-22T07:15:36.795Z
---
...
A software that respond to user requests by serving web pages. Can deliver static content (like fixed HTML files) or run programs called CGI (Common Gateway Interface) scripts. CGI programs dynamically generate HTML content based on user input or database interaction, allowing websites to provide customized, interactive responses.
...
A detailed, chronological record of user activities and system transactions, capturing who did what, when, and where. Provides accountability, transparency, and help detect unauthorized or fraudulent activities.
...
Databases widely support encryption to protect data. Encryption can be applied at different levels:
...
A signed document that verifies something's authenticity. Produced by a Certificate Authority (CA). Public keys of CA can be used to verify the authenticity.

Essential for verifying the authenticity of public keys in secure communications. Commonly used by websites and browsers. Browsers have pre-installed CA certificates to verify websites' authenticity.
