...
lastUpdatedOn: 2026-09-13T04:35:59.802Z
---
...
- Wired hosts  
  Servers or fixed systems.
- Communication network  
  Wi-Fi, 4G, 5G, Bluetooth.

Communication can either be:

- Through [base stations](/s3/data-communication-and-networking/mobile-telecom/#base-station)
- Direct host-to-host
...
- Query optimization  
  Minimize data transferred and network cost.
- Energy consumption  
  Transmission uses more energy than receiving.
- Location services  
  Queries depend on device location.
- Data broadcast  
  Base stations broadcast shared data.
- Disconnection  
  Must maintain cached local data and sync later.
- Consistency  
  Local copies can become outdated.
...
- Energy  
  Minimize transmission usage. Reception requires much less energy than transmission.
- Timing  
  Off-peak vs peak network costs.
...
- Recoverability  
  Updates may be lost.
- Consistency  
  Local data may not match server state.
...
If all `Vd,i[k] \le Vd,j[k]` and not equal → `i`’s copy is older.
...
- Synchronization is async, event-driven, and conflict-aware.
...
- There are 2 architectures, lightly embedded (external engine) and deeply embedded (in-process).
