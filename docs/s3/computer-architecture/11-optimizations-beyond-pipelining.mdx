...
lastUpdatedOn: 2026-09-13T04:35:59.598Z
---
...
The processor's ability to execute multiple instructions from a single program simultaneously. Achieved through [pipelining](/s3/computer-architecture/pipelining) and similar techniques. Used at architecture level.
...
- Implementation  
  Time-sharing or multiple processors.
- Simultaneous Multi-threading (Hyperthreading)  
  Uses multiple virtual cores on a physical core.

A technique to run multiple [threads](/s3/operating-systems/thread/) concurrently. Using 1 or more cores.
...
[RISC-V](/s3/computer-architecture/risc-v-architecture)'s atomic extension includes instructions for multithreading.
