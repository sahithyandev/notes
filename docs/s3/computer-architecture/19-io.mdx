...
sidebar:
  order: 19
slug: s3/computer-architecture/io
prev: true
next: true
dateCreated: 2026-04-22T07:09:10.689Z
lastUpdatedOn: 2026-09-21T13:41:33.609Z
---
...
- Human-readable  
  Monitor, printer.
- Machine-readable  
  Disks, sensors, actuators.
- Communication  
  Remote devices, other computers.
...
CPU [polls](/s3/operating-systems/io-system#polling) the devices repeatedly and waits until the operation is complete. Simple to implement (no extra interrupt hardware or handlers required). Predictable control flow and timing.
Wastes clock cycles.
...
CPU continues its operation. When the I/O device is ready, it [interrupts](/s3/operating-systems/io-system#interrupt-driven) the CPU. Saves the current context (to the stack), handles the interrupt, restores the context (from the stack), and resumes execution. More efficient than programmed I/O. Requires special hardware.
...
1. Multiple interrupt lines  
   Each module has a dedicated line.
2. Software poll  
   CPU checks modules sequentially (slow).
3. Daisy chain (hardware poll)  
   Modules connected in a chain; the first ready device handles it.
4. Vectored interrupt  
   Device sends a vector (address of its service routine).
5. Bus arbitration  
   Device must first control the bus to request an interrupt.
...
2. I/O controller added, enabling **programmed I/O**.
3. **Interrupts** introduced, making I/O more efficient.
4. **DMA** added, enabling block transfer without the CPU.
