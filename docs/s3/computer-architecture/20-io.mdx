...
CPU [polls](/s3/operating-systems/io-systems/#polling) the devices repeatedly and waits until the operation is complete. Simple to implement (no extra interrupt hardware or handlers required). Predictable control flow and timing.
Wastes clock cycles.
