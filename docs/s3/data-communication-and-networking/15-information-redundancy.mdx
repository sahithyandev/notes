...
lastUpdatedOn: 2026-06-04T19:53:54.360Z
---
...
Suppose a hamming code has $d$ data bits and $r$ parity bits. Total codeword length is $d + r$ bits.

$r$ parity bits can encode $2^r$ distinct states. Each state maps to one outcome:

- $d + r$ states: one per bit position in the codeword, indicating which bit is erroneous.
- $1$ state: no error.

So $2^r$ must cover all $d + r + 1$ outcomes:
