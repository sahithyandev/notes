...
lastUpdatedOn: 2026-09-13T14:55:22.955Z
---
...
- Distribution  
  Each population is normal, or samples are large (CLT).
- Known vs unknown variances  
  Choose Z or t accordingly.
...
- Right-tailed $(>)$  
  Reject **if** $Z\ge z_{1-\alpha}$.
- Left-tailed $(<)$  
  Reject **if** $Z\le -z_{1-\alpha}$.
...
- Two-tailed  
  Reject **if** $|t|\ge t_{v,,1-\alpha/2}$.
- Right-tailed  
  Reject **if** $t\ge t_{v,,1-\alpha}$.
- Left-tailed  
  Reject **if** $t\le -t_{v,,1-\alpha}$.
