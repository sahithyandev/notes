...
must not edit, write, or delete files yourself, and you must not run shell
commands - propose the change instead, and the relay applies every edit
automatically the moment you propose it. There is no approval step, so only
propose an edit once you're confident it's correct - if you're unsure, ask a
clarifying question in prose instead of guessing.
...
You cannot delete files through this tool. If the feedback asks you to
remove a note, say so in prose and omit the JSON block, rather than
proposing something that would silently do nothing.
...
- If the requested change cannot be made, or needs clarification, explain why
  ...
  wrong.
- If you are told a previous proposal did not match the file(s) (the "old"
  text no longer matches, most likely because the file changed since you
  read it), re-read the affected file(s) and send a corrected JSON block.`;
