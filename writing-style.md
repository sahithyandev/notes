...
18. Never use em dashes. Restate or elaborate inline with a colon or parentheses.
...
- No em dashes: restatement uses a colon or parentheses instead
...
| Distinctive marks | "Aka.", fragment-with-period, bold key terms, `<Note>` blocks     |
...
Cache hit: requested data is found in cache. Cache miss: data must be fetched from main memory, which is expensive. Hit rate $h$ and miss penalty $t_m$ determine effective access time:
