...
- `src/integrations/live-edit/toolbar-app.ts` - a native **Astro dev toolbar app** ("Live Edit", registered via `addDevToolbarApp()` in `index.ts`'s `astro:config:setup` hook, entrypoint resolved relative to that file) for whole-note feedback ("tighten this note") or a cross-note merge ("fold this into that note"): click its toolbar icon to open a window, pick one or more notes via the `+ note` file picker (backed by `GET /__live-edit/notes`, defaulting to the current note if you're on one), write a comment, send. Submits a `kind: "whole-note"` request with a `files` array. This panel only covers these whole-note/merge items, since (unlike a selection) they have no single spot in a note's body to anchor to. Its own `init(canvas, app)` builds the window's DOM by hand (no Astro component templating inside a toolbar app - `canvas` is a bare `ShadowRoot`), and calls `app.toggleNotification(...)` to badge the toolbar icon while a proposal is waiting on you.

`liveEdit()` is registered in `astro.config.mjs`'s `integrations`, next to `notesStyleValidator()`; the per-note widget is rendered directly by `[...slug].astro`. The two widgets share item updates via a dispatched `le-item` DOM event rather than both opening their own SSE connection.
