...
- `OpenCodeAdapter` spawns one one-shot `opencode run --format json --agent live-edit` process per turn instead (opencode's `run` takes the message as an argv and persists conversation state server-side via `--session`, not over stdin), and always reads the final reply back via `opencode export <sessionId>` afterward rather than trusting the run stream to carry it - `run --format json`'s NDJSON shape isn't as fully documented as Claude's, so progress events parsed from it are best-effort only (a miss just means a step doesn't show up in the UI's "Working: ..." trail). Its tool restrictions are written to an auto-generated `.opencode/agent/live-edit.md` (gitignored, regenerated from the same `SYSTEM_PROMPT` on first use, not hand-duplicated) rather than passed as CLI flags.
...
A backend's `checkReady(spawnFn)` (built from `readyIf()` and shared `runCliCapture()`/`checkCliVersion()` helpers in `agent.ts`) determines whether it shows up at all: every backend needs its CLI to actually run (`<bin> --version`), and opencode additionally needs at least one configured provider (`checkOpenCodeHasProvider()` greps `opencode providers list`'s "N credentials" summary - opencode has no `--format json` for that subcommand, and its own free `opencode/*` models are blocked outright for headless use regardless, so "the CLI runs" alone isn't enough). Checked once at server startup, not per-request. With no backend available, the widgets show a disabled message (naming each backend's reason) instead of a disabled toggle; with more than one, `live-edit-bar.astro` renders a `<select>` built from `GET /__live-edit/status`'s `backends` map (never a hardcoded option list) that posts to `POST /__live-edit/backend` on change.
