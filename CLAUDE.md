...
- `src/components/dev/live-edit-bar.astro` - site-wide, mounted by `Layout.astro`, always present in the page (present on every page, note or not). A floating bar for whole-note feedback ("tighten this note") or a cross-note merge ("fold this into that note"): pick one or more notes via the `+ note` file picker (backed by `GET /__live-edit/notes`, defaulting to the current note if you're on one), write a comment, send. Submits a `kind: "whole-note"` request with a `files` array. Its own review panel only covers these whole-note/merge items, since (unlike a selection) they have no single spot in a note's body to anchor to.

Both widgets check `src/integrations/live-edit/enabled-state.ts` (a shared `localStorage`-backed flag, `"live-edit-enabled"`, on by default) and hide themselves when it's off. The Astro **dev toolbar** carries a "Live Edit" entry (`toolbar-app.ts`, registered via `addDevToolbarApp()` in `index.ts`'s `astro:config:setup` hook) that is a pure on/off switch for that flag - clicking its icon just calls `app.toggleState()`/reacts to `app.onToggled()`, it never opens a canvas window or hosts any UI of its own. (An earlier version of this feature put the whole-note bar's UI inside the toolbar app's own canvas; that stayed cramped even after widening it, so the UI moved back out to being always-present on the page, with the toolbar reduced to just the toggle.)
