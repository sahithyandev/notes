...
`src/integrations/live-edit/` is a dev-only Astro integration (not a raw Vite plugin, so `astro:server:setup` gives it a proper `AstroIntegrationLogger` - its terminal output is timestamped and labeled `live-edit` the same way `module-redirects` and `notes-style-validator` are, not a bare `console.log`) that lets you attach feedback to notes in the browser and get a proposed edit back, without leaving the browser. Nothing ships in a production build (it only acts on `astro:server:setup`, which never fires during `astro build`). Two widgets feed it, both mounted directly on the page (not inside the dev toolbar's own canvas):

- `src/components/dev/live-edit.astro` - per-note, rendered by `[...slug].astro` under `import.meta.env.DEV`. No button of its own: selecting text in the article auto-attaches it as context to `live-edit-bar.astro`'s shared input (once the selection settles on mouseup/touchend/keyup, not while dragging, and after a short delay so a quick click-and-release doesn't attach anything) by dispatching `le-attach-selection`, shown there as a chip. This widget keeps only the live DOM `Range` it captured (needed to anchor a marker in the article, which the bar can't do), and reacts to `le-selection-submitted` (insert the marker once the bar's Send actually posts a `kind: "selection"` request) and `le-selection-detached` (the bar's chip was removed - drop it). Progress and the eventual applied/error result render inline in the article right after the selected text (a small marker inserted at the end of the captured Range) - not in a corner popup. There is no review/approval step: an edit applies straight to disk as soon as the agent proposes it (see "Backends" below), so this marker shows the outcome, not a proposal awaiting a click.
- `src/components/dev/live-edit-bar.astro` - mounted by `Layout.astro`, but only rendered on note pages (`Layout`'s `isArticle` check, true only when `authors` are passed - only `[...slug].astro` does that), so it never shows on the homepage or a semester overview. The one shared input for both flows: whole-note feedback ("tighten this note") or a cross-note merge ("fold this into that note") - pick one or more notes via the `+ note` file picker (backed by `GET /__live-edit/notes`, defaulting to the current note if you're on one, labeled "Current file" rather than its path) - and, when a selection is attached (see above), a `leb-chip-selection`-styled chip showing a preview of the selected text, removable via its own × (dispatches `le-selection-detached`). A pending selection takes priority on Send: it submits a `kind: "selection"` request instead of `"whole-note"`, and reports the result back via `le-selection-submitted` for `live-edit.astro` to render inline at the selection's marker, rather than in this bar's own status strip, since (unlike a selection) a whole-note/merge item has no single spot in a note's body to anchor to.

Both widgets check `src/integrations/live-edit/enabled-state.ts` (a shared `localStorage`-backed flag, `"live-edit-enabled"`, on by default) and hide themselves when it's off. The Astro **dev toolbar** carries a "Live Edit" entry (`toolbar-app.ts`, registered via `addDevToolbarApp()` in `index.ts`'s `astro:config:setup` hook) that is a pure on/off switch for that flag - clicking its icon just calls `app.toggleState()`/reacts to `app.onToggled()`, it never opens a canvas window or hosts any UI of its own. (An earlier version of this feature put the whole-note bar's UI inside the toolbar app's own canvas; that stayed cramped even after widening it, so the UI moved back out to being always-present on the page, with the toolbar reduced to just the toggle.) Because `toggleState()` marks the toolbar app "active" for as long as live-edit stays enabled - the default - Astro's own toolbar bar would otherwise never auto-hide itself again after the first click (its mouseleave handler skips the auto-hide timer while any app is active); `toolbar-app.ts` works around this by reaching into the toolbar's ShadowRoot and calling its `setToolbarVisible(false)` directly after every toggle. `live-edit-bar.astro`'s own bottom-docked UI (the bar, its status strip, the disabled notice) also stays clear of Astro's toolbar via a `--le-toolbar-clearance` CSS custom property, since the toolbar's hover hit-region is full-width and would otherwise swallow clicks meant for them.

`liveEdit()` is registered in `astro.config.mjs`'s `integrations`, next to `notesStyleValidator()`. The two widgets share item updates via a dispatched `le-item` DOM event rather than both opening their own SSE connection.

### Backends

Live edit can drive either a headless **Claude Code** or **opencode** session - whichever is installed and actually usable, with a picker in the bar to choose when more than one is. `src/integrations/live-edit/agent.ts`'s `BACKEND_DEFINITIONS` array is the single place a backend is registered (`{ id, label, bin, createAdapter, checkReady }`); `index.ts` drives detection, selection/persistence, and the web UI's picker entirely off that list and never names a backend directly. Adding a provider means writing its `AgentAdapter` implementation and adding one entry - nothing else changes.

- `ClaudeStreamAdapter` runs one long-lived `claude -p --input-format stream-json --output-format stream-json` process, fed one user message per feedback request over its stdin, so the conversation and prompt cache carry across requests instead of being rebuilt each time. The agent can only `Read`/`Glob`/`Grep`/`Skill`; it never gets `Edit`/`Write`, and **`Bash` is explicitly in `--disallowedTools`, not just left out of `--allowedTools`** - a scoped pattern like `Bash(bun run check-notes-style*)` in `--allowedTools` does not actually restrict Bash to that command in headless/print mode (verified with a raw stream-json probe against the CLI: it grants Bash outright, silently ignoring the pattern), so Bash must be excluded outright rather than "scoped."
- `OpenCodeAdapter` spawns one one-shot `opencode run --format json` process per turn instead (opencode's `run` takes the message as an argv and persists conversation state server-side via `--session`, not over stdin), and always reads the final reply back via `opencode export <sessionId>` afterward rather than trusting the run stream to carry it - `run --format json`'s NDJSON shape isn't as fully documented as Claude's, so progress events parsed from it are best-effort only (a miss just means a step doesn't show up in the UI's "Working: ..." trail). It runs opencode's **default agent**, not a custom one: the free `opencode/*` tier (no credentials needed) 403s any session whose agent carries a `deny` `permission:` entry, so a restricted agent is incompatible with "works on the free tier" and hard tool restrictions were dropped for that backend (the `SYSTEM_PROMPT`, inlined into each session's first message since there's no `--agent` to carry it, is instruction-only enforcement - weaker than Claude's `--allowedTools`). `checkOpenCodeHasProvider` was removed for the same reason: the CLI running is enough.

Both agents' replies end with the same fenced ` ```json ` proposal block, `{ summary, changes: [{ file, edits: [{ old, new }] }], deletions: [file, ...] }` - `changes` can list more than one file for a merge, `deletions` lists whole files to remove outright (e.g. "delete this note and the next 2"). `proposal.ts` parses and validates it (each `old` must match its file's current content exactly once) and `index.ts`'s `handleResult()` writes every edit in `changes` straight to disk immediately - there is no review or approval step, and correspondingly no "proposed" or "discarded" item status (see `ItemStatus` in `index.ts`). If an `old` string doesn't match (the file changed since the agent last read it), the relay asks the agent to re-propose once with the mismatch details (`buildMismatchMessage` in `prompt.ts`); a second failure is a terminal error for that item. `deletions` are parsed but **never applied** - the system prompt tells the agent not to propose any, and if one shows up anyway (e.g. a resumed session running an older cached system prompt, see the session-resumption note below) it's logged and ignored, not executed. There is no delete route, no `confirmDelete`, and no `resyncAfterDeletion` - deleting a note through Live Edit is not currently possible.

Progress (queued, each agent tool-use step, applied/error, style-check follow-ups, the child process's own stderr) is logged to the terminal running `bun dev`, not just to the browser.

A backend's `checkReady(spawnFn)` (built from `readyIf()` and shared `runCliCapture()`/`checkCliVersion()` helpers in `agent.ts`) determines whether it shows up at all: every backend needs its CLI to actually run (`<bin> --version`); opencode needs nothing more, since the free `opencode/*` tier now serves it (no credentials required). Checked once at server startup, not per-request. With no backend available, the widgets show a disabled message (naming each backend's reason) instead of a disabled toggle; with more than one, `live-edit-bar.astro` renders a `<select>` built from `GET /__live-edit/status`'s `backends` map (never a hardcoded option list) that posts to `POST /__live-edit/backend` on change.

Routes on the dev server (`localhost:4321`):

- `GET /__live-edit/status` - `{ available, backend, backends }`; each backend's `{ label, available, reason? }`.
- `GET /__live-edit/notes` - every note's path/slug/title, for the bar's file picker.
- `POST /__live-edit/request` - submit a new feedback item (`kind: "selection"` or `"whole-note"`); its edits apply automatically once the agent proposes them, there is no separate apply/discard/refine route.
- `POST /__live-edit/backend` - switch the active backend (`{ backend }`); disposes the current adapter, the other backend's session is untouched.
- `POST /__live-edit/reset` - drop the current backend's agent session and start a new one.
- `GET /__live-edit/events` - SSE stream of item state.

Each backend keeps its own session id, persisted to `.tmp/live-edit-session-<backend>` (gitignored) so a dev-server restart resumes the same conversation instead of starting cold, and switching backends doesn't lose either one's conversation. The developer's backend choice itself is persisted to `.tmp/live-edit-backend`; on the next startup it's used as long as that backend is still available, otherwise falling back to whichever is (preferring `BACKEND_DEFINITIONS`' order - currently Claude first - when nothing's been chosen yet). **Resuming a Claude session keeps its original system prompt** even though `agent.ts` passes the current `SYSTEM_PROMPT` on every spawn - a `claude` CLI behavior, not a bug here - so if you change `SYSTEM_PROMPT` (e.g. the proposal JSON contract) and want a running dev session to pick it up, `POST /__live-edit/reset` (or delete the relevant `.tmp/live-edit-session-*` file before starting `bun dev`) instead of just restarting the server. `LIVE_EDIT_MODEL` overrides the model for whichever backend is active; `LIVE_EDIT=0` disables the plugin entirely.
